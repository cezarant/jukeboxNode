  ...
  http.listen(port, function(){        
	...
	usbDetect.on('add', function(device){ 
         	console.log('Pen Drive conectado...'); 	        
	});
	
	usbDetect.on('remove', function(device){ 
		console.log('removido...');  
		intervalId = setInterval(my, 1500);   		
	});
  ...
  function listaArquivos(){
	...
	if (fs.existsSync('itens.json'))
	    ...
	    fs.writeFile("dicionario.json", JSON.stringify(dicionario), function(err, result) {
  ...
  function classificaBandas(){
     ...
     console.log(found); 	
     var fs = require('fs');
     fs.writeFile("itens.json", JSON.stringify(itens), function(err, result) {
	    if(err) console.log('error', err);
  ...
  function classificaAlbuns(){
     for(var j = 0;j< itens.length;j++){
         for(var i = 0;i< itens[j].bandas.length;i++){ 	     
             if((itens[j].bandas[i].nome === itens[j].artist) && (itens[j].album !== undefined)){
		   itens[j].bandas[i].albuns.push({ nome: itens[j].album, musicas: []});
	     }  	             	
	 }    
     }	
     classificaMusicas();
  ...
  function classificaMusicas(){
     ...
     var fs = require('fs');
     fs.writeFile("thing.json", JSON.stringify(itens), function(err, result) {
	    if(err) console.log('error', err);
